...
3. **Do it**: code → build (`node --check kumm.mjs`) → deploy → test on the harness (`node kumm.mjs check --json / .\Deploy-ModPack.ps1 -Verify -PackDir <pack>; full table in AGENT_GUIDE.md "Test harness"`),
