...
4. **Build** (`node --check kumm.mjs`). If errors — fix them, don't commit broken state.
5. **Deploy/run** as your project requires.
6. **Verify autonomously** on the harness (`node kumm.mjs check --json / .\Deploy-ModPack.ps1 -Verify -PackDir <pack>; full table in AGENT_GUIDE.md "Test harness"`). Look at the result carefully — don't
