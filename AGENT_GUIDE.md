...
<!-- [AI] Checked at the 2.7 update (2026-09-18): the owner's own «СТАРАЕМСЯ / СТАРАЕТСЯ» IS the effort verb — the rendering stands, nothing reworded. -->
...
| Gaming panel | **3840×2160 @ 144 Hz** — a TV ("Beyo"), driven by the 5070 Ti. This is the ceiling that makes frame-rate targets meaningful: the game's `FrameRateLimit=141` is correctly just under it, and any generated frame above 144 is paid for but never shown. | `Get-CimInstance Win32_VideoController \| ? CurrentHorizontalResolution \| % {$_.CurrentRefreshRate}` |
| FPS measurement | **PresentMon 1.8.12407** at `C:\Program Files\NVIDIA Corporation\FrameViewSDK\bin\PresentMon_x64.exe` (arrives with the NVIDIA stack, not on PATH). **Needs an elevated shell** — it captures through ETW. Records per-frame timings to CSV: `--process_name Palworld-Win64-Shipping.exe --output_file <csv> --timed <sec> --no_console_stats --stop_existing_session`; `--hotkey ALT+SHIFT+F11` records only the marked stretch (F8–F11 are taken by the mods). This is what replaces "on the eye" FPS talk — the pack's `Pareto-audit.md` carried "замеров нет" as its first caveat. | `& '<path>' --help`; `[Security.Principal.WindowsPrincipal]::new([Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole('Administrator')` |
...
Filing a ticket about a defect of KAIF ITSELF (the feedback contour, "A defect in KAIF ITSELF") is ONE
move, and it is the named carve-out of the `AUTH:` gate (Git workflow above; KAIF 2.7, origin #65):

```
node .kaif/kaif-core.mjs report bugs/KAIF/NN_*.md          # add --dry-run first to read what would go out
```
...
| `node tools/kaif-update-sweep.mjs <old bundle> <new bundle>` | AFTER every KAIF update pass, before the first checkpoint and again after the hand merges: names upstream lines that did not reach the disk (the task shows only what the classifier noticed — origin #72). Must end with exit 0 |
...
| `node tools/test-guards.mjs` | the self-test of both guards below: 36 cases (the heredoc hook, the claim guard in file and staged modes, the whole pre-commit on a rename); red on their first versions. Run it after touching either guard |
