...
> Тема: владелец вычитывает предложенный перевод 234 строк мода и решает три вопроса формы.
...
> Статус: **✅ первая версия принята владельцем 2026-09-12 10:41 +03:00** («ок, первая версия принята»)
> после четырёх правок в чате: строка 28, правило «X Values» = «Значения X» (11 строк), «Броня»
> вместо «Доспехи», строка 35 «Тип температуры». Дальше — опыт с `.locres` (STATUS, эстафета п. 1)
...
- Из 294 уникальных **246 — настоящие подписи интерфейса** (ниже, 234 строки таблицы: несколько однотипных строк сведены в одну). Остальные 48 — образцы
...
## A. Окно настроек — заголовки разделов и переключатели (122 строки)
...
| 35 | Temperature Type  | Тип температуры | ✅ правка владельца 12.09 |
...
## B. Окно настроек — подсказки под переключателями (48 строк)
...
## D. Группа, экран и прочие надписи (22 строки)
...
## E. Не переводить — образцы конструктора и внутренние имена (48)
...
**Ответ:** A — «ок, первая версия принята» (2026-09-12, чат), с правками, внесёнными в таблицу.
...
**Ответ:** A — принято вместе с версией целиком (отдельно не обсуждалось; откат дёшев — четыре строки).
...
**Ответ:** A — принято вместе с версией целиком (отдельно не обсуждалось).
...
## Добавление №1 — строки из байткода Blueprint, принято владельцем 2026-09-12 ~15:00

**Почему их не было в основной таблице.** Разборщик `ftext_extract.py` искал `FText` только в
данных ассета (с префиксом длины) и проходил мимо второй формы — литералов, вкомпилированных в
БАЙТКОД Blueprint (`EX_TextConst` → source, key, namespace, без префиксов длины). Их 145 ключей и
65 строк сверх прежних 794/294. Разборщик починен, `hosav-ftext.csv` пересобран: 939 ключей.

Ниже — то, что владелец согласовал сразу, по его вопросу «а класс брони Light перевести можешь?».
Остальные строки этой формы — отдельным файлом на вычитку, `interview_003_hosav_hud_bytecode.md`.

| № | EN | RU | примечание |
|---|---|---|---|
| 235 | Light | Лёгкая | класс брони в шапке HUD; согласовано с «броня» (правило владельца из этой же таблицы) |
| 236 | Medium | Средняя | там же |
| 237 | Heavy | Тяжёлая | там же |
| 238 | {armortype} / {armor}{suffix} / {damagereduction}% dr  | {armortype} / {armor}{suffix} / {damagereduction}% сниж.  | шапка брони; `dr` = damage reduction, владелец выбрал «сниж.» |
| 239 | {damage}{suffix1} / {arrmorpen}% / {damagetotal}{suffix2} dps  | {damage}{suffix1} / {arrmorpen}% / {damagetotal}{suffix2} ур/с  | шапка урона; `dps` → «ур/с» по слову владельца |

Фигурные скобки — подстановки движка, их трогать нельзя: имена внутри `{}` перенесены дословно.
