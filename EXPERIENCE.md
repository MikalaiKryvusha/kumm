...
### EXP-0112 · 2026-09-16 · ❌→✅ · #debug #hypothesis #control #conan #ue5 #umg #bug12
**Context:** баг 12 — модалка выбора перка мертва (пункты не жмутся, «Позже» не закрывает). Журнал давал почти приговор: наш мод паузы поставил `SetGlobalTimeDilation(0.0001)` за 1,6 с до открытия модуля `PerkSelection`, и дальше 26 секунд ни одной смены состояния.
**Tried / did:** вместо того чтобы чинить по совпадению во времени, пошёл искать МЕСТО, ГДЕ ТОТ ЖЕ МЕХАНИЗМ РАБОТАЕТ. Прочитал в некукнутом контенте кита класс кнопок модалки (`W_PerkSelection` → `WBP_ButtonBase` → делегат `SignalClicked`) и затем меню ESC (`W_IngameMenu`) — оно собрано на ТЕХ ЖЕ `WBP_ButtonBase` и том же `SignalClicked`. А меню ESC под 0.0001 жмётся: и с модом PauseGame из Мастерской, и в нашем журнале 13.09 23:16.
**Result:** ✅ ведущая гипотеза опровергнута за десять минут чтения диска, без единого запуска игры. Заодно так же опровергнута вторая («мод уровней стёр перки»): ванильная `DT_AttributeSystem` имеет ровно те три колонки, что мы пишем.
**Lesson:** **совпадение во времени — это не причина, и опровергается оно дешевле, чем подтверждается: ищи КОНТРОЛЬ — место, где подозреваемый механизм действует, а симптома нет.** Если тот же класс виджета под тем же воздействием работает в другом окне, «воздействие убивает виджет» как общий закон мёртв, и чинить по нему — значит потратить сборку и час владельца на неверную правку. Контроль ищется по ОБЩЕЙ ЧАСТИ (здесь — класс кнопки и имя делегата), а не по внешнему сходству окон.
  → link: `bugs/12_conan_perk_dialog_dead_under_pause.md` · [[EXP-0074]] · [[EXP-0111]]
**Repro:** назови класс сломанного элемента, потом `grep`-ом найди другие ассеты с тем же классом: `python -c "import re;print([n.decode() for n in re.findall(rb'[ -~]{4,}',open(PATH,'rb').read()) if b'Button' in n])"` по `F:\CEDevKit\CEUE5Devkit\UE4\Content\**\W_*.uasset`. Работает ли контроль — смотреть в журнале/у владельца, а не предполагать.
**Trigger:** журнал показал, что твой мод действовал за секунды до симптома, и ты собираешься это чинить.
**Not for:** случаи, где подозреваемый механизм уникален (единственное место применения) — контроля просто нет, и остаётся A/B.
