...
1. ✅ **ЗАКРЫТО 2026-08-21** по указанию владельца «вычисти вирусы и зловреды». `restore.ps1` в
   наборе `2325` переписан: явный список из восьми разрешённых записей, отказ по `removed != true`,
   отказ на весь класс `defender-exclusion`, отказ по префиксу `mal-` и по именам вредоносных
   политик; пробный прогон по умолчанию, боевой требует `-Execute` и повышенных прав. Проверено
   прогоном: 8 восстановит, 30 откажет, 2 пропустит. Оригинал лежит рядом как
   `restore.ps1.ORIGINAL-UNSAFE` — он остаётся уликой к разделу 5.6 и **запускать его нельзя**.
   Образцы зловреда вынесены в `_MALWARE_DO_NOT_RESTORE\` с разбором того, что они делали.
   Машина проверена отдельно и чиста — подробности в описании ниже по тексту не менялись, проверка
   21.08 добавлена в `STATUS.md`.
   *Ниже сохранено исходное описание дефекта — оно объясняет, ПОЧЕМУ пришлось переписывать:*
   скрипт не проверял поле `removed` и **создал бы шестнадцать исключений антивируса на путях
   зловреда**, которых уборка даже не снимала, плюс вернул бы шесть зловредных задач вперемешку с
   легитимными. От срабатывания спасал только другой дефект — падение на 22-й записи из-за
   отсутствующего `mal-task-5.xml`.
...
- `2026-08-17_2246`: `orphan-drweb-cureit` — `%USERPROFILE%\Doctor Web`, 1 434 571 534 Б.
...
```
28967888   <steamid>\ER0000.sl2
28967888   <steamid>\ER0000.sl2.bak
    2394   GraphicsConfig.xml
```
`%USERPROFILE%\AppData\Roaming\EldenRing` не существует. Поиск `ER0000.sl2*` и `*.sl2*` по всем пяти локальным дискам (C, D, E, F, J — включая `J:\OLD_HDD`) дал единственное попадание: эту самую пару в карантине. Самой игры нигде не нашлось (обход C/D/E/F/J на глубину 4).
...
#     F:\_QUARANTINE_C\2026-08-17_2335\orphan-eldenring-roaming  ->  %USERPROFILE%\AppData\Roaming\EldenRing
#   DRY RUN - nothing was changed. Add -Execute to act.
...
Copy-Item -LiteralPath 'F:\_QUARANTINE_C\2026-08-17_2335\orphan-eldenring-roaming' `
          -Destination '%USERPROFILE%\AppData\Roaming\EldenRing' -Recurse
```
Побочное: SteamID папки — `<steamid>`, это accountID 1638, подозрительно ранний номер. Взгляни сам, твоё ли это сохранение, прежде чем считать его незаменимым.
...
```powershell
$sc = '\\?\GLOBALROOT\Device\HarddiskVolumeShadowCopy1\Users\<user>\AppData\Roaming\obs-studio\basic\profiles\Krinik\basic.ini'
...
`lib\Common.ps1` держит два списка. `$BlockedExact` перечисляет `%USERPROFILE%\AppData\Roaming`, `...\AppData\Local`, `C:\ProgramData` — и комментарий над ним прямо объявляет:
