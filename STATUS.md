...
0в. 🟢 **ИДЕЯ 03 — Проворство без потолка растит бег и прыжок. ПРОДОЛЖАТЬ ЗДЕСЬ.** Вайб: **«ничто не
   ...
   стоп-сигнал). Разбор — раздел «Шаг 2, утро 13.09» в `ideas/03_conan_atributy_kak_v_morrowind.md`.
   ...
   - ✅ **урон от падения**: пороги `MinDamageVelocity` −1000 / `DeadlyFallVelocity` −2400 на герое; растить их
     тем же множителем — прыжок x8 (57 м) без урона (`EXP-0106`). x8: бег 3600 ед/с, разгон ~0.8 с.

   **Дальше, атомарно:** (1) собрать мод из трёх доказанных механизмов; форму роста от Проворства
   показать владельцу вариантами на числах, не выбирать самому; (2) `MaxAcceleration`/торможение тем же
   множителем; (3) спринт синтетическим Shift не включается — причина открыта; (4) **мод камеры:
