...
377 строк) и `researches/nvidia-tuning/` (конспект по драйверу, 490 строк).
...
- 📦 **Три вещи ждут возврата из карантина** (пробные прогоны выполнены, боевые НЕ запускались):
      ...
      на C: не возвращать).
- 🖥️ **Конспект по настройкам NVIDIA готов** — `researches/nvidia-tuning/README.md` (490 строк,
...
### Состояние машины на момент передачи (21.08 ~11:15 +03:00)
...
| Настройки NVIDIA | заводские после «Восстановить»; **G-SYNC выключен вручную** (телевизор мерцает на VRR — не включать обратно) |
| HDR рабочего стола | **ВКЛЮЧЁН** и, вероятно, владельцем не замечен (в игре HDR выключен — это другой переключатель). Кандидат №3 конспекта |
| Задача автозапуска RTSS | **отсутствует**, снята уборкой; копия цела |
| Файл подкачки | **на C:**, 4096–65536 МБ, автоуправление снято, вырос до 14.4 ГБ |
| Parsec | возвращён (агент убивал его в ходе проверки) |
| Карантин уборки | `F:\_QUARANTINE_C`, 9.4 ГБ, **ничего не восстанавливалось** |
| Оба дерева git | чистые и запушены: KUMM `cadb511`, пак `89b3b81` |
...
2. **Доложить владельцу конспект по NVIDIA** — `researches/nvidia-tuning/README.md` готов и проверен; пять первых действий перечислены в «Awaiting human review» выше.
