...
2. 🟢 **Проверить глазами то, что раскатано, но не смотрено:** `zoom_min = 30` (заказ «ближе перед
   переходом в FPV» — не слишком ли близко), `hud_scale = 1.25` (не велик ли),
   `ApplicationScale = 1.1` на плотных окнах.
3. 🟡 **Дождь (пункт 4)** и 🟡 **русификация (пункт 6)** — следующие по величине. У пункта 6
