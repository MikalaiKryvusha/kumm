...
> **Вечер 12.09 — ночь 13.09.2026 — идея 03 из теории в опыт.** Dev Kit безоконно отдал адрес
> атрибута и формулу скорости (граф `BasePlayerChar` в T3D + трассировщик `t3d_trace.py`); зонд
> `ConanAttributeProbe` читает атрибуты и пишет базу бега; все шесть номеров сверены вложением очков.
> Прочитан вайб владельца из Unliminium. Снять чат через `Input.ini` не вышло. Подробности — пункт 0в.
...
0в. 🟢 **ИДЕЯ 03 — Проворство без потолка растит бег и прыжок. ПРОДОЛЖАТЬ ЗДЕСЬ** (владелец 13.09
   ~00:45: «завтра продолжим»). Вайб: **«ничто не упирается»**, как Oblivion с SimpleUncappedAttributes
   — источник `D:\work\unliminiumProject\AGENT_GUIDE.md`, блок «ВАЙБ»; «потолок / кап / насыщение» в
   своих словах — стоп-сигнал. Весь разбор — конец `ideas/03_conan_atributy_kak_v_morrowind.md`. Коротко:
   - ✅ атрибуты читаются: `ConanCharacter:GetIntStat(id, 0)`, Проворство = 19 (сверено вживую, `EXP-0097`);
   - ✅ формула скорости прочитана из графа героя: `MaxWalkSpeed = NewSpeed (база RunSpeed/WalkSpeed
     из C++) × перк Agi15 × замедления × перегруз`. **Мод меняет БАЗУ**, проценты игры ложатся сами —
     так и хочет владелец. `MovementSpeedOverride` — не рычаг (обходит перегруз);
   - 🟡 запись `RunSpeed` x2 проходит и игра пересчитывает итог, но **замер разбега на сытом герое не сделан**;
   - 🟡 `JumpZVelocity` игра сбрасывает на 420, блюпринт героя его не пишет — виновника искать в ките.

   **Порядок, атомарно (шаг · кадр глазами · журнал):**
   1. **Убрать чат** — владельцу он не нужен, а Numpad * (локальный), Numpad − (клан) и Enter его
      открывают. `-ActionMappings` в `Saved/Config/Windows/Input.ini` НЕ работает (проверено, откачено).
      Пробовать меню игры «Настройки → Управление» со снимками, потом Lua.
   2. Зонд `ConanExiles/_unpacked/ConanAttributeProbe` (раскатан в `ue4ss/Mods/`): **Alt+Numpad .** —
      атрибуты, движение, здоровье; **Alt+Numpad *** — бег и прыжок x2; **Alt+Numpad /** — вернуть.
      ⚠️ Alt+* открывает чат, пока он не снят. Голый нумпад и Ctrl+нумпад заняты камерой — перед любой
      новой привязкой сверять `bind(` / `bind_ctrl(` в её `main.lua`.
   3. Замер: Alt+. → `input.ps1 -Key 0x57 -HoldMs 3000` → Alt+. → разница координат, до и после x2.
      Ориентир: сытый герой — MaxWalkSpeed 325 (голодный — 263).
   4. Кит: кто пишет `JumpZVelocity` — `researches/conan-devkit/bp_speed.py` (выгрузка T3D, UTF-16) +
      `t3d_trace.py <файл.utf8.t3d> <Функция>`.
   Игру запускать `Start-Process`, **не** `enter-world.ps1` целиком; «Продолжить» — клик 127,328; копия
   сохранения перед записью — `D:\Games\Conan Exiles Mods\_save-backup\`.
