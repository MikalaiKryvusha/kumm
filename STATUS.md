...
5. **Подобрать хвост конспекта.** При закрытии 08.09 два ворквлоу ещё дописывали разделы, и их
   файлы могли лечь в репозиторий **уже после последнего коммита**. Первая команда в паке Конана:
   `python _config/check-digest.py` — покажет, какие семьи появились (ждём `net`, `stream`,
   `debug-db`) и полны ли они; затем `git status` и коммит. Там же может лежать
   `docs/04-камера.md`.
