...
5. ✅ **Конспект `dw.*` дописан 08.09.2026 — 264 имени из 264, все девять семей.** Ворквлоу
   прошлой сессии не успел ничего (дерево было чистым, `origin...HEAD` = 0 0), три семьи
   написаны заново. **Главное в них не разделы, а метод:** внешние источники по этим 60 именам
   пусты (одно имя из шестидесяти на три дампа), зато **бинарник игры несёт справку самих Funcom
   к каждому cvar-у** — снята для 58 из 60 (`EXP-0041`, правило 2-бис в `docs/dw-cvars/README.md`).
   Правило 2 канона уточнено наблюдением: `dw.*` приезжают из **четырёх** секций ini, а не из
   одной.

6. Проверка каркаса — `node .kaif/kaif-core.mjs check`.

### 🆕 Изменилось 2026-09-08 (вторая сессия) — то, что меняет поведение

1. **Конспект `dw.*` закрыт** (пункт 5 выше). Метод, который стоит переиспользовать: справка
   Funcom к каждому cvar-у лежит **в бинарнике** — `EXP-0041` и правило 2-бис в
   `ConanExiles/docs/dw-cvars/README.md`.
2. **`verify-install.py` починен** — он кричал «БИТО 2, ОТСУТСТВУЕТ 78» на ЗДОРОВОЙ установке
   (бонусы установки + подменённая библиотека). Теперь: «ТРЕВОГ НЕТ», **код возврата 0/1**, база объявленных
   исключений с закреплёнными хешами, сторож `never_excepted`, флаг `--raw` (`EXP-0042`).
   **Проверка самой заглушки:** `python _config/test-verify-baseline.py` — 13 проверок, оба
   контроля. Это первый исполняемый тест где-либо в периметре.
3. **Каталог модов Nexus снят** — `ConanExiles/docs/05-каталог-модов-nexus.md`, все 40 со
   счётчиками. Nexus по Enhanced почти пуст: 40 модов, 18 из них у одного автора, **ни одного**
   под три заказа. `Yerts Conan Lumen Enhanced` (№37) помечен как **конфликт**: это чужой
   `Engine.ini`, он затрёт нашу сборку, и деплой не поймёт причины.

**Проверено наблюдением:** игра **ни разу не запускалась** после правок конфигов — последний
выход 08.09 в 00:01:58 (`ConanSandbox.log`), наши записи позже (00:41…01:04); `deploy-config.py
--dry-run` даёт 0 различий, хотя игра переписывает ini при каждом выходе.

### 🎯 Три заказа владельца — состояние (его формулировка, 08.09.2026)

| заказ | состояние |
|---|---|
| **графика** | применено в файлы, **ни разу не проверено в живой игре** |
| **камера** | только разведка (`docs/04-камера.md`), ничего не поставлено |
| **глубокая симуляция мира** («чтобы движок работал как на мультиплеерных серверах») | материал собран: семьи `ai-lod`, `net`, `stream`. **Ключевой вывод:** одиночная игра — listen-сервер, репликация в ней молчит (некому слать), поэтому рывки на базе идут НЕ оттуда. Порядок: сперва стриминг (`dw.WCStreamAroundAllClientsInListenServer`), потом тик-рейты — настраивать AI LOD в выгруженном мире бессмысленно |
| **моды** | `"mods": []`. Каталог Nexus снят и почти пуст. **Мастерская Steam не снята — без входа отдаёт 479 КБ разметки без единой карточки; нужен Steam-аккаунт владельца в отладочном Chrome** |
