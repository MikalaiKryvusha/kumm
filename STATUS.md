...
0а. 🔴 **ГЛАВНОЕ НА СЛЕДУЮЩИЙ ЧАТ — `plans/05_conan_nexus_publication.md`.** Владелец решил
   выложить наш мод камеры на Nexus (под Enhanced такого мода нет — проверено). Готовы README
   мода, страница на двух языках, схема и промпт логотипа. **Два блокера, оба красные:**
   (1) **ось вращения не удерживается** — «пружинкой назад тянулась»; удержание дописано в
   сторож и раскатано, но **В ИГРЕ НЕ ПРОВЕРЕНО**; (2) **кадры негодны** — «вплотную» оказался
   видом от первого лица, три кадра наклона одинаковы, панорамы сверху вниз нет вовсе.
   ⚠️ Слово владельца на будущее: **«снимай и смотри глазами, получил ли ты то, что ХОТЕЛ
   получить»** — урок `EXP-0081`, память `look-at-what-you-made`.
