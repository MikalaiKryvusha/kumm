...
❓ **Первое лицо не накручивается колесом (16.09 22:27, НЕ ПРОВЕРЕНО до конца).** Владелец: «не могу
вид от первого лица накрутить» → сам же заметил: «когда лук в руках — игра не даёт вид от первого
лица поставить» → «видимо, это особенности игры». **Встречный факт, который надо развести:** наш
`ConanCameraControl` держит жёсткий пол `zoom_min = 30.0` (`config.lua`) и по комментарию в
`Scripts/main.lua:1317` проверяет предел «на каждом проходе, кто бы ни двигал число». Игра
докладывает свой `ThirdPersonCameraMinArmLength = 30.0`, штанга весь вечер лежала ровно на 30.0.
**Разводится одним опытом за пять секунд:** убрать лук из рук и покрутить колесо. Первое лицо
появилось — мод чист, это правило игры для дальнобойного; не появилось — виноват `zoom_min`, и это
одна строка в `config.lua` (там же `-1` = не трогать родное значение). Заводить баг ТОЛЬКО после
этого опыта: пока это не дефект, а неразведённая развилка.
