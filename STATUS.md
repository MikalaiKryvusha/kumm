...
2. 🟢 **Ждёт глаза владельца:** `zoom_min = 30` (ближе перед FPV) · **переменный шаг зума
   5 вблизи → 20 вдали** (`zoom_step_near`/`zoom_step_far`) · множители опыта подняты вшестеро ·
   `hud_scale = 1.25` · `ApplicationScale = 1.1` на плотных окнах.

2а. 🔴 **ОТКРЫТАЯ ЖАЛОБА 10.09:** вблизи героя камера **отодвигается после прыжка и спринта**;
   вдали этого нет. Не гадать — мод каждую секунду печатает
   `ЗУМ: штанга | цель | ДистанцияВозврата`, и по трём числам видно, кто двигает: игра сбросом
   режима или наш же мод, зеркалящий ей своё число (этот класс уже был, см. комментарий у
   `ThirdPersonCameraDefaultTargetArmLength` в моде).
