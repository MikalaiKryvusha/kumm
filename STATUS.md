...
- ✅ **Интервью #002 — перевод мода «Hosav's Custom UI» ПРИНЯТ владельцем 2026-09-12 ≈01:00** («ок, первая версия принята», четыре правки внесены в таблицу). Следующий шаг — не вычитка, а ОПЫТ: эстафета п. 1.
...
1. 🟢 **ГЛАВНОЕ — локализация мода Hosav по слову владельца («завтра сделаем локализацию»).**
   Таблица ПРИНЯТА (`interviews/interview_002_hosav_ru_perevod.md`, колонка RU — источник перевода;
   правило владельца: ближе к букве оригинала — «Values» → «Значения», «Type» → «Тип», «Броня»).
   Опыт по `researches/conan-devkit/README.md` §7.1, путь 1: собрать `ru`-`.locres` по 794
   ключам из `researches/conan-devkit/hosav-ftext.csv` (ключи 32-hex, пространство имён пустое;
   формат `.locres` v3 уже разобран в `glossary.py` — писатель делается из читателя), положить в
   свой pak по пути `Content/Localization/<Цель>/ru/` и проверить в игре снимком окна настроек
   Hosav. Первая развилка — под какую ЦЕЛЬ локализации игра подхватит чужой `.locres` (одну из 18
   своих или новую) — решается опытом, а не рассуждением. Не подхватит — путь 2: Lua-слой UE4SS
   с `SetText` по именам виджетов (уже масштабирует этот HUD).
   ⚠️ Игра отвергает пакеты НОВЕЕ себя, а кит новее игры: перед куком любого нашего пака — сверка
   версий `.uheader` (план 06, фаза 1, шаг 0). Для `.locres` кук не нужен: файл кладётся в пак
   `UnrealPak`-ом напрямую — проверить, что игра монтирует такой пак без `ModCompat.bin`.
