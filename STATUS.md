...
>
> 🔴 **Главное за ночь 09→10.09: зум камеры сломан НЕ нашим модом, и вечер ушёл не туда.**
> Слово владельца: «пока модов из стима не наставили, а был только наш рукописный мод вчера — всё
> хорошо было с зумом. Был лимит, не было рывков». 08.09 работало, 09.09 в 15:29–16:14 встали
> 36 модов Мастерской — сломалось. Агент семь раз правил СВОЙ мод, хотя журнал каждый раз
> показывал, что наши записи проходят. Разбор — `bugs/09`, урок — `EXP-0073`.
>
...
владельца «будем потиху в разных чатах чинить это»). Состояние на 10.09.2026 00:40:
**закрыты и приняты владельцем** — HUD Hosav увеличен (пункт 1) и камера после смерти (пункт 3),
плюс отдельным заказом интерфейс +10%. **Красным остался один** — зум камеры (пункт 2), и он
вынесен целиком в `bugs/09_conan_zoom_runaway.md`. Предыдущая очередь — `plans/03_conan_next_session.md`.
...
### 🗡️ Первым делом в новой сессии (после ночи 2026-09-10)

**Читать не этот раздел, а два документа:** `plans/04_conan_ochered_zakazov.md` (очередь заказов,
восемь пунктов) и `bugs/09_conan_zoom_runaway.md` (единственная незакрытая красная задача).

1. 🔴 **ЗУМ КАМЕРЫ — единственное красное.** Причина НЕ найдена, но круг сужен измерением, и весь
   разбор лежит в `bugs/09`. **Следующий шаг размечен: деление сборки пополам.** Выключить 18 из
   35 оставшихся модов Мастерской (`"enabled": false` в `modpack.json`), раскатать ЦЕЛИКОМ (баг 06
   запрещает `-Only`), владелец запускает и крутит колесо, агент читает журнал. 5–6 запусков.
   **Не переискивать то, что уже снято измерением** — список в `bugs/09`, раздел «Что НЕ надо
   переискивать». Главное из него: перехваты UE4SS по функциям блюпринта в этой сборке НЕ РАБОТАЮТ.
2. 🟡 **`Player Ignores Follower Collision` сейчас ВЫКЛЮЧЕН** — это был опыт, он невиновен.
   Включить обратно: убрать `"enabled": false` в `modpack.json`, раскатать целиком.
   Сейчас в игре 43 мода вместо 44, и `-Verify` из-за этого честно ругается
   `[FAIL] pak not in modlist` — это ожидаемо, а не поломка.
3. 🟡 **Перки.** Прежний вывод «из самой игры» СНЯТ. Перковые таблицы везут ДВА мода
   (`MrS_LevelAndAttributeOverhaul` и `EnhancedArmoryAndArsenal`), и дерево Авторитета со
   скриншота — ровно `DT_MrS_PerksTable_Authority`. Следующий шаг — назвать победителя по журналу
   игры, потом слово владельца. Подробности — `plans/04`, пункт 5.
4. 🟢 **Проверить глазами то, что раскатано, но не смотрено:** `hud_scale = 1.25` (не велик ли),
   `ApplicationScale = 1.1` на плотных окнах (инвентарь, карта, дерево перков — на 1.25 там
   вылезало), `zoom_interp_speed = 1.5` (не вязок ли зум на обычных дистанциях).

**Приборы, которых не было вчера:**

- `researches/ue5-iostore-index/utoc-index.py` — каталог ассетов ЛЮБОГО пака, включая моды
  Мастерской (они обёртки: внутри три контейнера IoStore). Читает все индексы, вывод в UTF-8.
  `python researches/ue5-iostore-index/utoc-index.py "<путь>/*.pak" > каталог.txt`
- Зонд интерфейса в `ConanModPackStatus`: `probe_widget_prefix` — выписывает живые виджеты мода и
  устройство их классов; `probe_world_classes` — расписывает классы, которые есть только В МИРЕ.
  Только метаданные, риска нет. Выключать пустой строкой после снятия отчёта.
- Сторож подмены в моде камеры: при старте печатает каждый ключ, где `config.lua` перебил код.
  Без него правка может не доехать до игры и выглядеть как «не помогло» (`bugs/08`).

**Три правила, оплаченные этой ночью:**

- **Сырой поиск строк по пакам ВСЕГДА ложноотрицателен** — содержимое сжато Oodle. Два прежних
  вывода проекта на нём и погорели. Искать только по офлайн-каталогу ИМЁН.
- **Спроси владельца «когда это работало в последний раз», ПРЕЖДЕ чем читать свой код** —
  один вопрос отделяет свою регрессию от чужого вмешательства (`EXP-0073`).
- **«Хук встал» ≠ «хук работает», «запись прошла» ≠ «значение держится»** — и то и другое
  проверяется счётчиком и обратным чтением (`EXP-0071`).
