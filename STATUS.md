...
> Одной строкой: **графика доведена и проверена владельцем, моды стоят, камера почти
> сделана — осталось убить пружинный возврат.** Это и есть единственная незакрытая нить.
...
#### 🎮 Проверить в игре (владелец ещё не смотрел)

3. **Тени**: `sg.ShadowQuality=4` дал группу `@Cine` (bias −1.5, 4096 страниц, SMRT 16/8).
   Владелец подтвердил только плавность солнца, качество дальних теней — нет.
4. **Замер производительности так и не снят.** Прибор готов и отлажен:
   ```
   bash /d/work/ai_sandbox/_tools/perf-harness/measure-conan.sh <метка>
   ```
   Сам ждёт запуска игры, входа в мир, снимает 25 секунд и разбирает `felt.py` + `analyze.py`.
   Имя ETW-сессии своё — `kumm-conan`.
5. **После выхода из игры** — два прогона в паке Конана:
   ```
...
| **камера** | 🟡 **почти**: «стало ровно за героем, как хочу», мешают пружинные возвраты. Два лекарства раскатаны, не проверены |
...
| **моды** | ✅ девять раскатаны, `modlist.txt` из порядка манифеста. Не проверены в игре |
