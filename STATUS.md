...
### 🔴 Первым делом (сессия 2026-09-16 ~21:00–21:50)

**1. Баг 12 — модалка выбора перка мертва, `bugs/12_conan_perk_dialog_dead_under_pause.md`.**
Слово владельца: «ни один из пунктов не нажимается и не выбирается, и диалог не закрывается по
кнопке Позже». Разведка по диску закончена, **две гипотезы опровергнуты, включая ведущую** —
диалог опознан (`W_PerkSelection_C`, «Choose A Perk» / «Choose Later»), а меню ESC собрано на тех
же `WBP_ButtonBase`/`SignalClicked` и под тем же замедлением 0.0001 **работает**. Мод уровней
перков не касается (три таблицы, ни одной перковой колонки). **Разведка по диску исчерпана —
дальше только A/B в игре**, порядок шагов записан в конце документа бага. Уцелевшие подозреваемые:
слои модалки под паузой и `ConanModPackStatus`, который двигал живые виджеты за секунду до неё.

**2. Кривая опыта поднята: якорь 175 → 500** (`ideas/04`, раздел «Кривая поднята»). Мод
`KrinikLevelUncapped` пересобран и раскатан, md5 `08dffe89b9a772f3d6059faf559b7a17` в паке и в
игре. **[NOT-TESTED] в игре.** Ожидание при первом входе: герой с 25 687 опыта был 36-го уровня →
станет **17-го**, до 18-го 2 121. Копии до правки в `_save-backup\` (`…2130.before-xp-x2-level36.db`,
`…2130.anchor175.pak`). Первым делом в журнале смотреть `extracting` вместо `skip extracting`.
...
🔴 **`bugs/12_conan_perk_dialog_dead_under_pause.md` — модалка выбора перка мертва (Конан, 16.09).**
Открыт, причина не найдена, разведка по диску исчерпана. Подробности — пункт 1 выше.

*(в коде самого движка KUMM открытых нет)* — закрытые: `bugs/01_DONE_*` · `02_DONE_*` · `03_DONE_*` ·
