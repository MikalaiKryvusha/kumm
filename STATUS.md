...
- ❓ **Идея 06 агента — два стража, ждёт вашего «да» или «нет»** (`ideas/06_strazhi_ot_lzhi_obolochki_i_chasov.md`):
      хук, который не пускает код с обратным слэшем через bash-heredoc (правка ваших настроек харнесса), и
      проверка в хуке коммита, что штамп времени не обгоняет часы. Повод — два класса моих ошибок, повторившихся
      18.09 вопреки записям в опыте (`EXP-0120`, `EXP-0121`).
...
Тикеты по самому фреймворку — все доставлены наверх. **Открыты два, оба найдены 18.09 на обновлении до 2.7:**
