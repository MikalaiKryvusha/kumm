...
Термин переименован ЛОКАЛЬНО: в навыках `baton` → `handover` (32 замены в обоих зеркалах,
`.claude/skills` и `.agents/skills`), в русских документах «батон» → «эстафета». Одно вхождение
