...
2. **Лампочка «моды загружены» в главном меню — ВЫКЛЮЧЕНА** (`ConanModPackStatus/enabled.txt.off`).
   ...
   **Мод ВКЛЮЧЁН 09.09 в безопасном режиме `mode = "probe"`** (`enabled.txt`, как у мода камеры):
   он ничего не создаёт и не показывает, только перечисляет метаданные классов-кандидатов и
   кладёт отчёт в `ue4ss/Mods/ConanModPackStatus/probe-report.txt`. От владельца нужен один
   запуск: постоять в главном меню ~15 секунд и выйти. По отчёту заполняются `class_path` и
   `text_field` в `config.lua`, после чего режим переключается на `show`.
