...
1. 🟡 **ГЛАВНОЕ — локализация мода Hosav по слову владельца («завтра сделаем локализацию»).**
   Ждать его правок в `interviews/interview_002_hosav_ru_perevod.md` (колонка RU + три «Ответ:»).
   Затем опыт по `researches/conan-devkit/README.md` §7.1, путь 1: собрать `ru`-`.locres` по 794
