...
1а. 🟡 **Локализация Hosav — состояние опыта 12.09** (разбор `researches/conan-devkit/README.md` §7.2):
   таблица ПРИНЯТА владельцем (`interviews/interview_002_hosav_ru_perevod.md`; его правило —
   ближе к букве: «Values» → «Значения», «Type» → «Тип», «Броня»). **Путь 1 (`.locres` поверх
   пака) ЗАКРЫТ загрузчиком игры** — «Filename … is not allowed in pak» (`EXP-0090`); до того
   съедена и грабля версии пака: `UnrealPak` кита пишет v12, игра читает 11, паки пишет
   `_tools/repak` (`EXP-0089`). **Путь 2 (Lua-мод `HosavRU`, подмена текстов виджетов по таблице)
   раскатан, синтаксис зелёный, В ИГРЕ НЕ ПРОВЕРЕН** — UE4SS мёртв. Приборы в паке:
   `_config/devkit/{locres.py, hosav-ru-entries.py, make-loc-mod.py}`, мод — `_unpacked/HosavRU/`
   (в `modpack.json` ещё НЕ внесён — вносить после проверки в игре). Штатный путь для пак-мода
   — скукованный Blueprint с `PolyglotDataToText` (фаза 1 эпика, после сверки версий).
