...
5. **Подобрать хвост конспекта — ПЕРВОЙ КОМАНДОЙ, до всего остального.**
   ```
   cd D:\work\ai_sandbox\ConanExiles && python _config/check-digest.py && git status
   ```
   На момент закрытия 08.09 ~02:20 готовы **шесть семей из девяти** и они полны (204 имени из
   204); `net` (18), `stream` (13), `debug-db` (29) дописывались ворквлоу **в момент закрытия** —
   их файлы и правки критиков могли лечь в репозиторий уже после последнего коммита. Что
   появилось — закоммитить; чего не появилось — дописать по образцу готовых, с пометками
   основания `[ИСТОЧНИК]`/`[ПО ИМЕНИ]`/`[АНАЛОГ]` и колонкой «как проверить».
   **Материал уже есть:** `_config/cvars-applied.txt` — штатные значения из журнала игры, включая
   всю семью `dw.nav.*` и `dw.OSD.Format`. И **в `DefaultEngine.ini` Конана над строками
   `dw.nav.*` стоят комментарии самих разработчиков** — первоисточник, найденный при правке
   раздела `nav-move`.
