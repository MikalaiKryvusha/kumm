...
➡️ **ОЧЕРЕДЬ РАБОТ — `plans/04_conan_ochered_zakazov.md`** (вечер 09.09, восемь пунктов
тикетами по слову владельца «будем потиху в разных чатах чинить»). Каждый пункт самодостаточен:
слова владельца, что УЖЕ ИЗМЕРЕНО, следующий шаг. Три красных: увеличить ТОЛЬКО HUD от Hosav ·
камера на большом отдалении · камера после смерти. Предыдущая очередь — `plans/03_conan_next_session.md`. Там всё разблокировано:
