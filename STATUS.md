...
### 🗡️ Первым делом в новой сессии (после ночи 2026-09-12)
...
0. 🩺 **ЕСЛИ МОДЫ «НЕ РАБОТАЮТ» — ПЕРВЫМ ДЕЛОМ ОДНА КОМАНДА, а не поиск своей ошибки:**
   ...
   ```
   Нашлось — **перезагрузить машину**. Дефект UE4SS (читает неинициализированную переменную и
   падает до первого Lua-мода); выглядит как поломка сборки. Разбор — `bugs/10`, уроки `EXP-0079/0080`.
   Сторож стоит в `_config/enter-world.ps1`. `UE4SS.log` стирается при КАЖДОМ запуске игры — улики
   снимать до перезапуска.
...
1а. 🔴 **UE4SS (`bugs/10`) — по-прежнему открыт, но БОЛЬШЕ НЕ БЛОКЕР локализации.** За 12.09:
   мёртвые 11:41, 12:39, 14:14 · живые 14:35, 14:41. Четыре кандидата сняты (разбор в багу):
   `bForceGUObjectArrayForIteration` опровергнут запуском — журнал с ним совпал с журналом без
   него ДОСЛОВНО; `DebugBuild`/`Stats` сняты самим журналом без запусков (движок уже определил
   `shipping` и `Off`); наш `VTableLayout.ini` сверен с апстримовым конанским — построчно совпал.
   Механизм назван по строкам DLL: выравнивание FName вычисляется динамически, у неудачи есть
   штатный запасной ход («defaulting to compile-time alignment»), и в наших падениях он НЕ
   срабатывает — код считает, что получил значение, и это ноль. «Здоровые» значения
   (`0x2684C788`, `0x997C788`, `0x74ABC788`) — усечённый указатель: одинаковые младшие 16 бит,
   в пределах одной загрузки машины значение постоянно. Следующий шаг с наибольшей отдачей —
   **тикет в RE-UE4SS** с двумя дословно совпадающими журналами, а не пятый ключ вслепую.
...
🔴 **`bugs/10` СНОВА ОТКРЫТ 12.09:** UE4SS падает на своей инициализации (`FName::StaticAlignment`)
и после перезагрузки машины тоже — «лечение» 10.09 было совпадением (`EXP-0091`). Пока он мёртв,
не работает НИ ОДИН Lua-мод сборки (камера, модалка, HUD). Чинить первым делом в новом чате;
веб-разведка по ошибке — `researches/conan-devkit/ue4ss-fatal-recon.md`, журнал падения сохранён
`researches/conan-devkit/ue4ss-fatal-2026-09-12-1239.log.txt`.
